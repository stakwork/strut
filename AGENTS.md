...
│   │   │   └── hive/      # hive/claim-pod + hive/release-pod: POST the pool manager (claim-pod/<workspaceId>, drop-pod/<workspaceId>?podId=&taskId=) with an x-api-token header; `hiveUrl`/`apiKey` config override the HIVE_URL/HIVE_API_KEY secrets (same fallback as create-pr's GITHUB_TOKEN). _client.ts: hiveAuth() resolves + validates both, never puts the key in an error
...
- **Actor secrets — a PERSON's credential, not the deployment's**
  ...
  `actor-secrets.json`, beside `secrets.json` (the default follows the
  deployment secret store's kind, not the workspace's, so a graph-backed
  host with file secrets keeps them across restarts; `createStrut({
  actorSecretStore })` overrides). Steps see nothing new: the runner binds the run's
